...
#include "LRUVertexCacheSimulation.h"
...
namespace
{
	std::ostream& printCacheReuse(std::ostream& out, bool output, const std::uint32_t* indices, std::size_t num_indices, std::size_t num_cached)
	{
		...
		if (output)
		{
			std::ofstream off("garnetLRU.txt", std::ios_base::app);
...
LRUVertexCacheSimulation::LRUVertexCacheSimulation(SceneSink& next, bool output, const std::size_t size)
	: ProcessingLayer(next), size(size), output(output)
{
}

void LRUVertexCacheSimulation::process(const vertex* vertices, std::size_t num_vertices, const std::uint32_t* indices, std::size_t num_indices, const surface* surfaces, std::size_t num_surfaces, const material* materials, std::size_t num_materials, const texture* textures, std::size_t num_textures)
{
	std::cout << "------------------ central LRU vertex cache simulation ------------------\n";
