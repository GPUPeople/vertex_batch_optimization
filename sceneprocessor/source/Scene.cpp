...
namespace
{
	...
	void buildAdjacency(const surface& surface, std::uint32_t* indices, const std::uint32_t* vertex_coord_map)
	{
		...
		{
			std::size_t operator ()(const halfedge_key_t& key) const
			{
		...
		std::unordered_map<halfedge_key_t, std::tuple<std::size_t, std::uint32_t>, halfedge_hash_t> edge_map;
...
//void exportScene(const char* filename, const vertex* vertices, std::size_t num_vertices, const std::uint32_t* indices, std::size_t num_indices, const surface* surfaces, std::size_t num_surfaces)
//{
...
void Scene::import(import_func_t& import_func, const char* begin, std::size_t length)
{
	import_func(*this, begin, length);
}

void Scene::importFrame(import_func_t& import_func, const char* begin, std::size_t length)
{
...
void Scene::finalize(bool nomaterial, bool mergeequalmaterials)
{
	...
	{
	std::size_t operator ()(const float3& p)
	{
	...
	std::unordered_map<float3, std::uint32_t, vertex_coord_hash_t> coord_vertex_map;
...
void Scene::addSurface(PrimitiveType primitive_type, std::vector<std::uint32_t>&& ind, const char* name, std::size_t name_length, const char* mat_name)
{
	//if (ind.size() % 3 != 0)
...
void Scene::addMaterial(const char* name, const float3& ambient, const float3& diffuse, const float4& specular, float alpha, const char* tex_name)
{
...
void Scene::process(SceneSink& processor, bool nomaterial, bool mergeequalmaterials)
{
	finalize(nomaterial, mergeequalmaterials);

	processor.process(&vertices[0], vertices.size(), &indices[0], indices.size(), &surfaces[0], surfaces.size(), materials.empty() ? nullptr : &materials[0], materials.size(), textures.empty() ? nullptr : &textures[0], textures.size());
}
