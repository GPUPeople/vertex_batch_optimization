...
namespace
{
	...
	void read_material(SceneBuilder& builder, const std::string& fname, const std::string& name)
	{
		...
		float3 ambient { 0.0f, 0.0f, 0.0f }, diffuse { 0.0f, 0.0f, 0.0f };
...
namespace obj
{
	void read(SceneBuilder& builder, const char* begin, std::size_t length)
	{
		...
		{
			std::size_t operator ()(const vertex_key_t& key) const
			{
	...
	void write(std::ostream& file, const vertex* vertices, std::size_t num_vertices, const std::uint32_t* indices, std::size_t num_indices, const surface* surfaces, std::size_t num_surfaces, const material* materials, std::size_t num_materials, const texture* textures, std::size_t num_textures)
	{
		...
		for (const surface* s = surfaces; s < surfaces + num_surfaces; ++s)
		{
			...
			for (std::uint32_t i = s->start; i < s->start + s->num_indices; i += 3)
				file << "f " << 1 + indices[i] <<'/' << 1 + indices[i] << '/' << 1 + indices[i] << ' '
				     << 1 + indices[i + 1] <<'/' << 1 + indices[i + 1] << '/' << 1 + indices[i + 1] << ' '
