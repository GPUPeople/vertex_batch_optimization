...
namespace sdkmesh
{
	...
	template <typename T>
	void readVertexAttribute(const char* buffer, std::size_t buffer_stride, T* dest, std::size_t dest_stride, std::size_t num_vertices)
	{
	...
	void read(SceneBuilder& builder, const char* begin, std::size_t length)
	{
		...
		for (std::size_t i = 0; i < header.NumMeshes; ++i)
		{
			...
			else
			{
				...
				for (std::size_t j = 0; j < mesh.NumVertexBuffers; ++j)
				{
					...
					for (int k = 0; k < MAX_VERTEX_ELEMENTS && vertex_buffer.Decl[k].Stream != 255U; ++k)
					{
						switch (vertex_buffer.Decl[k].Usage)
						{
							case USAGE_POSITION:
								...
								{
									readVertexAttribute<float3>(begin + vertex_buffer.DataOffset + vertex_buffer.Decl[k].Offset, vertex_buffer.StrideBytes, &(dest->p), sizeof(vertex), vertex_buffer.NumVertices);
								}
								break;
							...
							case USAGE_TEXCOORD:
								...
								{
									readVertexAttribute<float2>(begin + vertex_buffer.DataOffset + vertex_buffer.Decl[k].Offset, vertex_buffer.StrideBytes, &(dest->t), sizeof(vertex), vertex_buffer.NumVertices);
								}
								break;
							...
							case USAGE_NORMAL:
								if (vertex_buffer.Decl[k].Type == TYPE_FLOAT3 && vertex_buffer.Decl[k].UsageIndex == 0 && vertex_buffer.Decl[k].Method == METHOD_DEFAULT)
								{
									readVertexAttribute<float3>(begin + vertex_buffer.DataOffset + vertex_buffer.Decl[k].Offset, vertex_buffer.StrideBytes, &(dest->n), sizeof(vertex), vertex_buffer.NumVertices);
								}
								break;
						}
					}
			...
			for (std::size_t j = 0; j < mesh.NumSubsets; ++j)
			{
				...
				if (subset.PrimitiveType == PT_TRIANGLE_LIST)
				{
					...
					if (index_buffer.IndexType == INDEX_16)
					{
						...
						for (std::size_t i = 0; i < subset.IndexCount; ++i)
