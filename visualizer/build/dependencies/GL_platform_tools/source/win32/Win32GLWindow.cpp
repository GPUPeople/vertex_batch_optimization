...
namespace Win32::GL
{
	...
	void Window::attach(::GL::platform::DisplayHandler* handler)
	{
		...
		if (display_handler)
		{
			...
			display_handler->resize(rc.right - rc.left, rc.bottom - rc.top, this);
			display_handler->move(rc.left, rc.top, this);
		}
	}
	...
	LRESULT Window::WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_DESTROY:
			if (display_handler)
				display_handler->destroy(this);
			break;

		case WM_LBUTTONDOWN:
			if (mouse_input)
				mouse_input->buttonDown(::GL::platform::Button::LEFT, LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_MBUTTONDOWN:
			if (mouse_input)
				mouse_input->buttonDown(::GL::platform::Button::MIDDLE, LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_RBUTTONDOWN:
			if (mouse_input)
				mouse_input->buttonDown(::GL::platform::Button::RIGHT, LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_LBUTTONUP:
			if (mouse_input)
				mouse_input->buttonUp(::GL::platform::Button::LEFT, LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_MBUTTONUP:
			if (mouse_input)
				mouse_input->buttonUp(::GL::platform::Button::MIDDLE, LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_RBUTTONUP:
			if (mouse_input)
				mouse_input->buttonUp(::GL::platform::Button::RIGHT, LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_MOUSEMOVE:
			if (mouse_input)
				mouse_input->mouseMove(LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_MOUSEWHEEL:
			if (mouse_input)
				mouse_input->mouseWheel(GET_WHEEL_DELTA_WPARAM(wParam), this);
			break;
		...
		case WM_KEYDOWN:
			if (keyboard_input && (lParam & (1U << 30U)) == 0)
				keyboard_input->keyDown(keycode(wParam, lParam), this);
			break;

		case WM_KEYUP:
			if (keyboard_input)
				keyboard_input->keyUp(keycode(wParam, lParam), this);
			break;
		...
		case WM_SIZE:
			if (display_handler)
				display_handler->resize(LOWORD(lParam), HIWORD(lParam), this);
			break;

		case WM_MOVE:
			if (display_handler)
				display_handler->move(static_cast<short>(LOWORD(lParam)), static_cast<short>(HIWORD(lParam)), this);
			break;
