...
namespace X11::GL
{
	...
	void Window::handleEvent(const XEvent& event)
	{
		...
		case ButtonPress:
			if (mouse_handler)
				mouse_handler->buttonDown(static_cast<::GL::platform::Button>(1 << (event.xbutton.button - 1)), event.xbutton.x, event.xbutton.y, this);
			break;

		case ButtonRelease:
			if (mouse_handler)
				mouse_handler->buttonUp(static_cast<::GL::platform::Button>(1 << (event.xbutton.button - 1)), event.xbutton.x, event.xbutton.y, this);
			break;

		case MotionNotify:
			if (mouse_handler)
				mouse_handler->mouseMove(event.xmotion.x, event.xmotion.y, this);
			break;

		case KeyPress:
			if (keyboard_handler)
				keyboard_handler->keyDown(static_cast<::GL::platform::Key>(XkbKeycodeToKeysym(display, event.xkey.keycode, 0, 0)), this);
			break;

		case KeyRelease:
			if (keyboard_handler)
				keyboard_handler->keyUp(static_cast<::GL::platform::Key>(XkbKeycodeToKeysym(display, event.xkey.keycode, 0, 0)), this);
			break;

		case ConfigureNotify:
			if (display_handler)
				display_handler->resize(event.xconfigure.width, event.xconfigure.height, this);
			break;
